...
class GPX {
    ...
    init(fileName: String = "Location Data for \(Date().toShortStyleString())") {
        ...
        file = File(fileName: fileName, in: .documentDirectory)// custom directory
        ...
        //DELETE
        file?.insertAtNewline(content: "insertatnewline", at: 2)
        file?.insertAtNewlineFromEnd(content: "insert from end", at: 1)
    }
}
